...
namespace Assignment4.Controllers
{
    public class BillingController : Controller
    {
        ...
        public IActionResult GetBillingItemsByCategory(int catId)
        {
            try
            {
                return Json(_dbConn.GetBillingItemsByCategory(catId));
            }
            catch (Exception ex)
            {
        ...
        public IActionResult GetCustomerDetailsById(int id)
        {
            try
            {
                return Json(_dbConfig.GetCustomerDetailsById(id));
            }
            catch (Exception ex)
            {
                return Json(ex.Message);
        ...
        public IActionResult SaveBill(Billing billData, string billItems)
        {
            try
            {
                var Data = JsonConvert.DeserializeObject<BillOrderItems[]>(billItems);
                var result = _dbConn.SaveBillData(billData, Data.ToList());
                return Json(result);
            }
            catch (Exception e)
            {
